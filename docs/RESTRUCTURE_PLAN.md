...
1. スコープ内(「人工知能の概要」〜「ディープラーニングの応用」+ 社会実装フロー)の全シラバス項目がカバーされ、**それが `npm run validate` で機械的に検証できる**
...
/app/
  scripts/
    validate.ts                 # 【新規】整合性・網羅性チェック(付録D)。npm run validate で実行
  src/data/
...
2. `app/src/data/syllabus.ts`(Phase 0 の確定版)、`app/src/data/timeline.ts` / `pipeline.ts` の**型と骨格のみ**(era/stage の ID と title。本文は Phase 2)
3. `app/scripts/validate.ts` を作成(チェック項目は付録D)。`tsx` を devDependency に追加し、`package.json` に `"validate": "tsx scripts/validate.ts"` を登録する
4. 見本カードを3枚整備: concept 1枚(dropout を拡張)、problem 1枚(vanishing-gradient を kind: "problem" 化)、person 1枚(新規。付録Aを流用してよい)
5. `npm run validate` と `npm run build` がエラーなく通ることを確認し、停止して報告
...
- **既存のインタラクティブデモ(16種)と既存ページを壊さない**。各フェーズの完了条件に `npm run build`(tsc + vite build)の成功を含める
...
- [ ] `npm run validate` が全チェックをエラー0で通過する
- [ ] `npm run build` が成功する(既存デモ・ページの非破壊)
...
- 各フェーズの完了条件に `npm run validate`(Phase 1 以降)と `npm run build` の成功を含める
