...
### catastrophic-forgetting と Phase 4 WARNING-0 目標の衝突(記録のみ・今は決定不要)

`catastrophic-forgetting`の対策(EWC: Elastic Weight Consolidation、リハーサル法等)はG検定シラバスの
明示キーワードに含まれておらず、試験スコープ外の可能性が高い。その場合、対策カードを正規の
出題根拠(教科書的事実)をもって新設できないため、このWARNINGは**Phase 4完了時点でも
恒常残留する可能性がある**。これは「WARNING 0を目標とする」という合格基準(Phase 3バッチ1レビュー#9)
と衝突しうる。以下2案を記録するのみとし、Phase 4着手時点で判断する:

- **案a**: 第8章(AIの法律と倫理、範囲外)のdraftスタブ先例に倣い、EWC等の対策カードをスコープ外の
  draftスタブとして作成し、`solves`エッジだけ張って警告を解消する(カード本文は最小限、
  試験に出ない旨を明記)。
- **案b**: 「対策がシラバス範囲外であるため恒常的にwarning残留を許容する」という例外を
  schema/RESTRUCTURE_PLAN.mdに明文化し、validateの合格基準側を「既知の例外を除きWARNING 0」に
  修正する。
